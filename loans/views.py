...
from .serializers import (
    ...
    LoanDocumentTypeSerializer,
    InstallmentSerializer,
    LoanDocumentStatusSerializer,
...
def loan_types_get_add(request):
    ...
    if request.method == "GET":
        ...
        return Response(
            {"message": "Loan type creation failed", "errors": serializer.errors},
...
def loantype_detail(request, id):
    ...
    if request.method == "GET":
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"message": "Loan type updated successfully", "data": serializer.data},
                status=status.HTTP_202_ACCEPTED,
            )
        return Response(
            {"message": "Loan type updating failed", "errors": serializer.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
        ...
        loantype.delete()
        return Response(
            {"message": "Loan type deleted successfullly"},
...
def loan_status(request):
    if request.method == "GET":
        try:
            ...
            return Response(
                {"Error": "Loan status not found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if serializer.is_valid():
            ...
            return Response(
                {
                    "message": "Loan status created successfully",
                    "results": serializer.data,
                },
                status=status.HTTP_201_CREATED,
            )
            ...
            return Response(
                {"message": "Loan status creation failed", "errors": serializer.errors},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def create_loan(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response(
            {"message": "Loan created succesfully", "results": serializer.data},
            status=status.HTTP_201_CREATED,
        )
    return Response(
        {"message": "Failed", "results": serializer.errors},
        status=status.HTTP_400_BAD_REQUEST,
    )
...
def get_loans_by_loan_id(request, loan_id):
    try:
        ...
        return Response(
            {"message": "Loan does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
...
def update_loan(request, loan_id):
    ...
    if new_status_id:
        try:
            ...
            return Response(
                {"message": "Invalid status ID"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    serializer = LoanSerializer(loan)
    return Response(
        {"message": "Loan updated successfully", "results": serializer.data},
        status=status.HTTP_200_OK,
    )
...
def get_loans_by_member_no(request, member_no=None):
    if request.method == "GET":
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"message": "Loan updated successfully", "data": serializer.data},
                status=status.HTTP_202_ACCEPTED,
            )
        return Response(
            {"message": "Loan updating failed", "errors": serializer.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def get_installments_by_loan_id(request, loan_id):
    try:
        ...
        return Response(
            {"message": "No Installments"}, status=status.HTTP_404_NOT_FOUND
        )
...
def loan_documents_by_loan_id(request, loan_id=None):
    if request.method == "GET":
        ...
        if serializer.is_valid():
            ...
            return Response(
                {
                    "message": "Loan document created successfully",
                    "results": serializer.data,
                },
                status=status.HTTP_201_CREATED,
            )
        return Response(
            {"message": "Loan document creation failed", "errors": serializer.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
        ...
        return Response(
            {"message": "Loan document updating failed", "errors": serializer.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )
        ...
        documents.delete()
        return Response(
            {"message": "Loan document deleted successfullly"},
            status=status.HTTP_204_NO_CONTENT,
        )
...
def documentTypes(request, id=None):
    if request.method == "GET":
        ...
        return Response(
            {
                "message": "Loan document type creation failed",
...
def document_status(request):
    if request.method == "GET":
        try:
            ...
            return Response(
                {"Error": "Document status not found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if serializer.is_valid():
            ...
            return Response(
                {
                    "message": "Document status creation failed",
                    "errors": serializer.errors,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def pay_loan(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response(
            {"message": "Loan paid successfully", "data": serializer.data},
            status=status.HTTP_201_CREATED,
        )
    else:
        return Response(
            {"message": "Loan payment failed", "errors": serializer.errors},
            status=status.HTTP_400_BAD_REQUEST,
        )


def get_disbursed_loan_by_member(request, member_id):
    try:
        disbursed_loan = Loans.objects.get(member=member_id, status="DISBURSED")
